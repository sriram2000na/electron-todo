...
// Retrieve from file to print the events
...
list.addEventListener('mouseover', (e) => {
  ...
  elems.forEach((ele,idx)=>{
    if (e.target === ele) {
      const li = ele
      const time = document.querySelectorAll("#time")[idx]
      const icon = document.querySelectorAll('#icon')[idx]
      const content=document.querySelectorAll("#content")[idx]
//    Delete icon
      ...
      icon.addEventListener('click',(e)=>{
        ...
        fs.writeFile(path.join(__dirname,'../../tasks.json'),newData,(err)=>{
          if(err){
            console.error(err); 
          }
          console.log(newData);
...
li.addEventListener('click',e=>{
  ...
  console.log('click');
  // content=document.querySelectorAll("#content")[idx]
  if(e.target!==icon){
    if(content.classList.contains("hider")){
      console.log("showing");
      content.classList.remove("hider");
      content.classList.add('shower');
      time.classList.add('hider');
      time.classList.remove("shower");
      // console.log(time);
      e.stopImmediatePropagation();
    }
    else{
      console.log("hiding");
      content.classList.remove('shower');
      content.classList.add('hider');
      time.classList.add('shower');
      time.classList.remove('hider');
      e.stopImmediatePropagation();
    }
  }
})
